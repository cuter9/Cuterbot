...
def calibrate_cam(pattern_images):
    ...
    cal_dim = img.shape[:2]  # dimension of image for calibration
    N_OK = counter  # number of applicable calibration images
    K = np.zeros((3, 3))
    D = np.zeros((4, 1))
...
def un_distorted(img, K, D, cal_dim, balance):
    ''' 焦距比例控制，控制视场大小
        https://medium.com/@kennethjiang/calibrate-fisheye-lens-using-opencv-part-2-13990f1b157f
    ...
    dim3 = dim1  # dim of input image for final un-distort image
...
def main():
    # calibrate fish camera
    dir_patterns = "D:\AI_Lecture_Demos\Data_Repo\Cuterbot_Repo\snapshots"
    pattern_images = os.path.join(dir_patterns, '*.jpg')
    K, D, cal_dim = calibrate_cam(pattern_images)
