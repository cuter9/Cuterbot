...
class DataCollection(HasTraits):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.xy_point_image = np.empty((self.height_display, self.width_display, 3), dtype=np.uint8).tobytes()
        self.bbox_image =  np.empty((self.height_display, self.width_display, 3), dtype=np.uint8).tobytes()
    ...
    def get_xy_point_image(self, x_slider_value, y_slider_value):
        image = cv2.resize(self.camera.value, (self.width_display, self.height_display),
                           interpolation=cv2.INTER_LINEAR)
        x = x_slider_value
        y = y_slider_value
        ...
        image = cv2.circle(image, (x, y), 4, (0, 255, 0), 2)
        image = cv2.circle(image, (int(self.width_display / 2), int(self.height_display)), 4, (0, 0, 255), 2)
        self.xy_point_image = bgr8_to_jpeg(image)

    def get_bbox_image(self, bbox):
        image = cv2.resize(self.camera.value, (self.width_display, self.height_display),
                           interpolation=cv2.INTER_LINEAR)
        x1 = int((np.min([bbox[0][0], bbox[1][0]]) * self.width_display / 2 + self.width_display / 2)[0])
        y1 = int((np.min([bbox[0][1], bbox[1][1]]) * self.height_display / 2 + self.height_display / 2)[0])
        x2 = int((np.max([bbox[0][0], bbox[1][0]]) * self.width_display / 2 + self.width_display / 2)[0])
        y2 = int((np.max([bbox[0][1], bbox[1][1]]) * self.height_display / 2 + self.height_display / 2)[0])
