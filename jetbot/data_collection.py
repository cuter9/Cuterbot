...
class DataCollection(HasTraits):
    ...
    def get_bbox_image(self, bbox):
        image = cv2.resize(self.camera.value, (self.width_display, self.height_display),
                           interpolation=cv2.INTER_LINEAR)
        x1 = int(np.min([bbox[0][0], bbox[1][0]]) * self.width_display / 2 + self.width_display / 2)
        y1 = int(np.min([bbox[0][1], bbox[1][1]]) * self.height_display / 2 + self.height_display / 2)
        x2 = int(np.max([bbox[0][0], bbox[1][0]]) * self.width_display / 2 + self.width_display / 2)
        y2 = int(np.max([bbox[0][1], bbox[1][1]]) * self.height_display / 2 + self.height_display / 2)
